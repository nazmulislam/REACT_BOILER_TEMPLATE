...
//const PHPConsole = path.resolve(__dirname + "/../../../web-app-api/Console.php");
...
function createFiles(moduleName, srcPath, destinationPath,prefix="") {
  ...
  fs.readdir(componentsDirectoryPath, function (err, files) {
    ...
    files.forEach(function (file) {
      let sampleFileName = file;

      let newFileName = sampleFileName.replace("Sample", pascalCase(moduleName));
...
function generatePhinxFile(moduleName) {
  runner.exec(
    PHP + " " + PHPConsole + " create:phinx-file " + pascalCase(moduleName),
...
function generateSeedFile(moduleName) {
  runner.exec(
    PHP + " " + PHPConsole + " create:seed-file " + pascalCase(moduleName),
    function (err, phpResponse, stderr) {
      if (err) console.log(err); /* log error */
...
function generateBackendFiles(moduleName) {
  runner.exec(
    PHP + " " + PHPConsole + " create:dmr-files " + pascalCase(moduleName),
    function (err, phpResponse, stderr) {
      if (err) console.log(err); /* log error */
    }
  );
  runner.exec(
    PHP + " " + PHPConsole + " create:unit-test-class " + pascalCase(moduleName),
    function (err, phpResponse, stderr) {
      if (err) console.log(err); /* log error */
    }
  );
...
function generateRbacFiles(moduleName) {
  runner.exec(
    PHP + " " + PHPConsole + " create:rbac-file " + pascalCase(moduleName),
    function (err, phpResponse, stderr) {
      if (err) console.log(err); /* log error */
    }
  );
