...
const Console = path.resolve(__dirname + "/../../../../../../../API/Console.php");

const PHP = process.env.PHP_INTERPRETER_PATH;

const APP_FOLDER_PATH = '../../../../../../src/'
...
function createFiles(moduleName, srcPath, destinationPath,prefix="") {
  ...
  fs.readdir(componentsDirectoryPath, function (err, files) {
    ...
    files.forEach(function (file) {
       ...
       */
      if(!fs.existsSync(componentsDestinationPath + "/" + newFileName))
      {
        fs.copyFile(
        ...
        console.log('The file '+componentsDestinationPath + "/" + newFileName + ' was not created as it already exists');
      }
...
function createReactComponent(moduleName,prefix) {
 ...
 createFiles(
  moduleName,
  "../Templates/Page",
  APP_FOLDER_PATH + "Pages"
);
appendToFile(
  APP_FOLDER_PATH + "Pages/index.ts",
  ...
  createFiles(
    moduleName,
    "../Templates/Module",
    APP_FOLDER_PATH + "Modules/" + pascalCase(moduleName),prefix
  );
  // appendToFile(
  //   APP_FOLDER_PATH + "Modules/index.ts",
   ...
   */
  appendToFile(
    APP_FOLDER_PATH + "store/Reducers/index.ts",
  ...
  insertIntoFileBySearchText(
    APP_FOLDER_PATH + "store/initialState.ts",
    initalStateData
  );
}
...
function createAppMenu(moduleName) {
   ...
   */
  appendToFile(
    APP_FOLDER_PATH + "Menus/index.ts",
    `${EOL}export * from "src/Modules/${pascalCase(moduleName)}/${pascalCase(
      moduleName
    )}Menu"${EOL}`
  );
  ...
  insertIntoFileBySearchText(APP_FOLDER_PATH + "Routes/appMenuItems.ts", appMenuItems);
}
...
function createComponentRoute(moduleName,prefix="") {
  ...
  //   moduleName,
  //   "../Templates/Route",
  //   APP_FOLDER_PATH + "Routes/privateRoutes/",prefix
  // );
   ...
   */
  appendToFile(
    APP_FOLDER_PATH + "Routes/privateRoutes/index.ts",
    `${EOL}export * from "src/Modules/${pascalCase(moduleName)}/${pascalCase(
      moduleName
    )}Routes"${EOL}`
  );
  ...
  insertIntoFileBySearchText(
    APP_FOLDER_PATH + "Routes/private-routes.ts",
    privateRoutes
  );
}
